...
const SearchNavbar = () => {
  ...
  return (
    <nav className="bg-[#00010D] shadow-sm py-3 fixed-search-navbar">
      <div className="max-w-7xl mx-auto px-4 relative search-container flex items-center">
        <img src="/segteclogo.png" alt="SegTec Logo" className="w-auto h-10 mr-4" />
        <div className="flex-grow flex justify-center relative"> {/* Added relative here */}
          <form onSubmit={handleSearchSubmit} className="flex items-center">
            <input
              type="search"
              placeholder="Buscar productos o servicios..."
              className="
                w-full max-w-lg px-4 py-2 border rounded-l-md bg-card-bg text-text-primary
          ...
          {showSuggestions && suggestions.length > 0 && (
            <ul className="absolute z-10 w-full max-w-md bg-card-bg border border-gray-300 rounded-md mt-1 shadow-lg"> {/* Removed left-1/2 transform -translate-x-1/2 */}
